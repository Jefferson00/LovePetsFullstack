...
export default function Home(props: HomeProps) {
  ...
  const handleToggleFav = async (pets_id: string) => {
    if (user) {
      try {
        ...
        if (myFavs) {
          const favToDelete = myFavs.find(
            (fav) => fav.pet_id === pets_id && fav.user_id === user.id
          );
  ...
  const openFilterOptions = () => {
    ...
    filterContainerRef.current.classList.add(
      styles.filterContainerMobileOpened
    );
  };
  ...
  const closeFilterOptions = () => {
    ...
    filterContainerRef.current.classList.remove(
      styles.filterContainerMobileOpened
    );
  };
  ...
  useEffect(() => {
    if ("geolocation" in navigator) {
      ...
      navigator.geolocation.getCurrentPosition((position) => {
        setCookie(
          undefined,
          "@LovePetsBeta:location_latitude",
        ...
        setCookie(
          undefined,
          "@LovePetsBeta:location_longitude",
      ...
      alert(
        "Desculpe, mas o seu navegador parece não possuir geolocalização. "
      );
    }
    if (user) {
      loadFavs();
  ...
  return (
    <div>
      ...
      <div className={styles.homeContainer}>
        {pets.length === 0 && !loading && <Default type="pets" />}
        {pets.map((pet) => {
          return (
            <Card
              key={pet.id}
        ...
        <div className={styles.filterContainer} ref={filterContainerRef}>
          ...
          <div className={styles.filterContent} ref={filterContentRef}>
            ...
            <span
              ...
              onClick={() => handleUpdateSpecieFilter(null)}
            >
              Limpar filtro
            </span>
            ...
            <div className={styles.genderFilterContainer}>
              ...
              <button onClick={() => handleUpdateGenderFilter(null)}>
                <IoMdMale size="14" color={!gender ? "#12baba" : "#c4c4c4"} />
                <IoMdFemale size="14" color={!gender ? "#12baba" : "#c4c4c4"} />
              </button>
            </div>
        ...
        <div className={styles.loadMoreContainer}>
          {hasMoreResults ? (
            ...
            <h4>Isso é tudo por enquanto.</h4>
          )}
        </div>
      </div>
    </div>
